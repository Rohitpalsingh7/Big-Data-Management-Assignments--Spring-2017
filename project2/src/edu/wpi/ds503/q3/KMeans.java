...
public class KMeans {
	...
	public static void main(String[] args) throws Exception {
		...
		String working_dir = "/kmeans_runs/"+unique_run+"/";

		Configuration conf = new Configuration();
		conf.set ("working_dir",working_dir);
		...
		for (int iteration=0; iteration < 5; iteration++){
			...
			String input_fname = working_dir + "clusters_input_"+iteration ;

			if (iteration == 0) {
				org.apache.hadoop.fs.FileSystem hdfs = org.apache.hadoop.fs.FileSystem.get(conf);

				Path path_clusters_input = new Path(input_fname+"/part-r-00000");
			...
			String output_fname = working_dir+"/clusters_input_"+(iteration+1);
			...
			//Long counter_value = job.getCounters().findCounter(KMeansReducer.UNCHANGED_COUNTER.VALUE).getValue();

			Long counter_value = job.getCounters().findCounter("STATUS", "unchanged").getValue();

			LOG.info("CHANGED or not?: "+String.valueOf(counter_value));
